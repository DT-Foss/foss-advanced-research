...
# Try importing TensorFlow, but don't crash if missing
try:
...
if tf:
    # Full Implementation if TF is present
    class ChaosEnhancedPolicyGAN(tf.keras.Model):
        ...
        def _build_chaos_conditioned_generator(self):
        ...
        def _build_hardened_discriminator(self):
    ...
    # Dummy Class if TF is missing (avoids usage errors in other imports, though not needed for main)
    class ChaosEnhancedPolicyGAN:
        def __init__(self):
            self.r_param = 3.99
...
if __name__ == "__main__":
    ...
    if tf:
        ...
        # Use the internal method if possible
